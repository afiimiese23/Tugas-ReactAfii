    ...
    <div className="p-8 bg-[#EEEEEE] min-h-screen font-['Poppins'] text-[#113D32]">
      ...
      <div className="flex justify-between items-center mb-8">
        <div className="flex space-x-8 border-b border-gray-200">
          {tabs.map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`pb-4 text-sm font-bold transition-all relative ${
        ...
        <div className="flex gap-3">
          <button
            ...
            className="bg-[#113D32] text-white px-10 py-2.5 rounded-xl flex items-center space-x-3 text-xs shadow-md">
            + New Rooms
          </button>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="text-[10px] tracking-widest text-black font-bold border-b border-gray-50 bg-white">
              ...
              <tr key={index} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors group">
                ...
                <td className="p-5">
                  <div className="flex items-center gap-4">
                    ...
                    />
                    <div>
                      <p className="text-[10px] text-[#113D32] font-black mb-1">{room.id}</p>
                      <p className=" text-[#6E6E6E]">{room.name}</p>
                    </div>
                  </div>
                </td>
                ...
                <td className="p-5">
                  ...
                  <p className=" text-gray-400">₹{room.rate} <span className="text-gray-400 font-normal">/night</span></p>
                </td>
                ...
                <td className="p-5">
                  <span className={`px-4 py-2 rounded-xl text-[10px] font-black text-center inline-block min-w-[85px] tracking-wider ${
                    room.status === "ACTIVE" 
                    ? "bg-[#68E365] text-[#ffffff]" 
                    : "bg-[#E23428] text-[#ffffff]"
                  }`}>
                    {room.status}
                  </span>
                </td>
